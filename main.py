...
def main():
    ...
    for k in features.keys():
        ...
        current_test_folder = os.path.join(base_test_folder, k)
        os.makedirs(current_test_folder, exist_ok=True)

        current_model_output_folder = os.path.join(base_model_output_folder, k)
        os.makedirs(current_model_output_folder, exist_ok=True)

        # 1. Caricamento dati
        df = load_data(categorical_columns, numerical_cols)

        baseline_cols = [t_1_visit]
        baseline = compute_baseline_vs_final(df, baseline_cols, output_folder=f"{current_test_folder}")

        # 2. Report valori null
        missing_report = generate_missing_report(df, current_test_folder)
        df = impute_nulls(log, df, categorical_columns)

        # 3. Preprocessing colonne
        df = convert_plus_minus(df)
        df, numeric_cols, categorical_cols = separate_columns(
            df,
        ...
        categorical_cols.remove(t_1_visit)  # rimuovo la colonna diagnosi prima visita

        # 4. Separazione X / y
        y = df[target_col].astype(int)

        X = df.drop(columns=[target_col])
        X = X.drop(columns=baseline_cols)  # rimuovo la colonna diagnosi prima visita

        results = correlation_analysis(X, y, output_folder=current_test_folder)
        X, removed_cols, numeric_cols, categorical_cols = drop_strongly_correlated(
            X, results['strong_corrs'], categorical_cols, numeric_cols
        )
        ...
        null_report.to_csv(f'{current_test_folder}/null_report.csv')

        # 5. Modelli
        models = get_models()
        res_df, trained_pipelines = train_models(log, k, X, y, numeric_cols, categorical_cols, current_test_folder)

        comparison_df = compare_models_to_baseline(
            res_df, baseline, trained_pipelines,
            output_folder=f"{current_test_folder}",
            model_output_folder=current_model_output_folder
        )
    ...
    log(f"\n✅ Test completato! Tutti i risultati sono in: {base_test_folder}")
    print(f"\n✅ Test completato! Tutti i risultati sono in: {base_test_folder}")
